...
if !place_meeting(x,y,obj_solid) && ((place_meeting(x,y+1,obj_solid) && argument[1]) or (!argument[1])) {exit;}
...
for (var gap = 0; gap <= limit; gap++) {
    for (var angle = 0; angle <= 360; angle += 5){
        ...
        if !place_meeting(xtarg,ytarg,obj_solid) {
            ...
            if argument[1] {if scr_NoJump(limit,xtarg,ytarg) {}
